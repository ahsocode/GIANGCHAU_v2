...
export function ChucVuCrud() {
  ...
  async function fetchItems(params?: { q?: string; sort?: string; order?: string }) {
    ...
    try {
      ...
      const res = await fetch(`/api/chuc-vu${query.toString() ? `?${query.toString()}` : ""}`);
          ...
          <div className="flex justify-end gap-2 pt-4">
            ...
            <Button
              ...
              onClick={async () => {
                ...
                try {
                  const res = await fetch(`/api/chuc-vu/${deleteTarget.id}`, { method: "DELETE" });
    ...
    <>
      <form
        ...
        onSubmit={(e) => {
          ...
          if (!ma || !ma.trim()) {
            toast.error("Vui lòng nhập mã chức vụ");
            return;
          }
          setConfirmOpen(true);
        }}
      >
        <div className="space-y-2">
          <label className="text-sm font-medium">Mã chức vụ</label>
          <Input
            value={ma}
            onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
              props.mode === "edit" ? setMaCustom(e.target.value) : setMaCustom("")
            }
            placeholder="VD: CV-01"
        ...
        <div className="space-y-2">
          <label className="text-sm font-medium">Tên chức vụ</label>
          <Input
            value={ten}
